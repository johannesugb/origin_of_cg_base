...
namespace cgb
{
	...
	void window::fill_in_extra_semaphore_dependencies_for_frame(std::vector<vk::Semaphore>& pSemaphores, uint64_t pFrameId)
	{
		for (const auto& [frameId, sem] : mExtraSemaphoreDependencies) {
			if (frameId == pFrameId) {
				pSemaphores.push_back(sem.mSemaphore);
			}
		}
	}
	...
	void window::fill_in_extra_render_finished_semaphores_for_frame(std::vector<vk::Semaphore>& pSemaphores, uint64_t pFrameId)
	{
		...
		for (auto i = si; i < si + mNumExtraRenderFinishedSemaphoresPerFrame; ++i) {
			pSemaphores.push_back(mExtraRenderFinishedSemaphores[i].mSemaphore);
		}
	}
	...
	void window::render_frame(std::initializer_list<std::reference_wrapper<cgb::command_buffer>> pCommandBuffers)
	{
		...
		cgb::context().logical_device().waitForFences(1u, &fence.mFence, VK_TRUE, std::numeric_limits<uint64_t>::max());
		cgb::context().logical_device().resetFences(1u, &fence.mFence);
		...
		cgb::context().logical_device().acquireNextImageKHR(
			...
			imgAvailableSem.mSemaphore, // The next two parameters specify synchronization objects that are to be signaled when the presentation engine is finished using the image [1]
			nullptr,
		...
		for (auto cb : pCommandBuffers) {
			cmdBuffers.push_back(cb.get().mCommandBuffer);
		}
		...
		std::vector<vk::Semaphore> waitBeforeExecuteSemaphores = { imgAvailableSem.mSemaphore };
		...
		std::vector<vk::Semaphore> toSignalAfterExecute = { renderFinishedSem.mSemaphore };
		...
		cgb::context().graphics_queue().submit(1u, &submitInfo, fence.mFence);

		// Present as soon as the render finished semaphore has been signalled:
		auto presentInfo = vk::PresentInfoKHR()
			.setWaitSemaphoreCount(1u)
			.setPWaitSemaphores(&renderFinishedSem.mSemaphore)
