...
vrs_image_compute_drawer::vrs_image_compute_drawer(std::shared_ptr<vkCommandBufferManager> commandBufferManager, std::shared_ptr<vulkan_pipeline> pipeline,
	std::vector<std::shared_ptr<vkCgbImage>> vrsDebugImages) : vkDrawer(commandBufferManager, pipeline), mVrsDebugImages(vrsDebugImages)
{
...
{
	vkContext::instance().device.destroyDescriptorPool(vrsComputeDebugDescriptorPool);
	vkContext::instance().device.destroyDescriptorSetLayout(vrsComputeDebugDescriptorSetLayout);
}

void vrs_image_compute_drawer::draw(std::vector<vkRenderObject*> renderObjects)
{
	...
	imgMemBarrier.image = mVrsDebugImages[vkContext::instance().currentFrame]->get_image();
	...
	imgMemBarrier.newLayout = vk::ImageLayout::eGeneral;
	imgMemBarrier.image = mVrsImages[vkContext::instance().currentFrame]->get_image();
	imgMemBarrier.subresourceRange.aspectMask = vk::ImageAspectFlagBits::eColor;
	imgMemBarrier.subresourceRange.baseArrayLayer = 0;
	imgMemBarrier.subresourceRange.layerCount = 1;
	imgMemBarrier.subresourceRange.levelCount = 1;
	...
	std::vector<vk::DescriptorSet> descriptorSetsToBind = { mDescriptorSets[vkContext::instance().currentFrame], mVrsComputeDebugDescriptorSets[vkContext::instance().currentFrame] };
...
void vrs_image_compute_drawer::createVrsComputeDescriptorSetLayout() {
	...
	if (vkContext::instance().device.createDescriptorSetLayout(&layoutInfo, nullptr, &vrsComputeDebugDescriptorSetLayout) != vk::Result::eSuccess) {
...
void vrs_image_compute_drawer::createVrsComputeDescriptorPool() {
	...
	poolSizes[0].descriptorCount = static_cast<uint32_t>(vkContext::instance().dynamicRessourceCount);

	vk::DescriptorPoolCreateInfo poolInfo = {};
	poolInfo.poolSizeCount = static_cast<uint32_t>(poolSizes.size());
	poolInfo.pPoolSizes = poolSizes.data();
	poolInfo.maxSets = static_cast<uint32_t>(vkContext::instance().dynamicRessourceCount);

	if (vkContext::instance().device.createDescriptorPool(&poolInfo, nullptr, &vrsComputeDebugDescriptorPool) != vk::Result::eSuccess) {
...
void vrs_image_compute_drawer::createVrsDescriptorSets() {
	std::vector<vk::DescriptorSetLayout> layouts(vkContext::instance().dynamicRessourceCount, vrsComputeDebugDescriptorSetLayout);
	vk::DescriptorSetAllocateInfo allocInfo = {};
	allocInfo.descriptorPool = vrsComputeDebugDescriptorPool;
	allocInfo.descriptorSetCount = static_cast<uint32_t>(vkContext::instance().dynamicRessourceCount);
	allocInfo.pSetLayouts = layouts.data();

	mVrsComputeDebugDescriptorSets.resize(vkContext::instance().dynamicRessourceCount);
	if (vkContext::instance().device.allocateDescriptorSets(&allocInfo, mVrsComputeDebugDescriptorSets.data()) != vk::Result::eSuccess) {
		throw std::runtime_error("failed to allocate descriptor sets!");
	}

	for (size_t i = 0; i < vkContext::instance().dynamicRessourceCount; i++) {
