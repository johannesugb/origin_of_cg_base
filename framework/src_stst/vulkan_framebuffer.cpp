...
namespace cgb {
	...
	vulkan_framebuffer::~vulkan_framebuffer()
	{
		for (size_t i = 0; i < swapChainFramebuffers.size(); i++) {
			vulkan_context::instance().device.destroyFramebuffer(swapChainFramebuffers[i], nullptr);
		}
		vulkan_context::instance().device.destroyRenderPass(renderPass, nullptr);
	}
	...
	void vulkan_framebuffer::createRenderPass() {
		...
		if (vulkan_context::instance().device.createRenderPass(&renderPassInfo, nullptr, &renderPass) != vk::Result::eSuccess) {
	...
	void vulkan_framebuffer::createFramebuffers() {
		...
		for (size_t i = 0; i < imagePresenter->get_swap_chain_images_count(); i++) {
			...
			if (vulkan_context::instance().device.createFramebuffer(&framebufferInfo, nullptr, &swapChainFramebuffers[i]) != vk::Result::eSuccess) {
