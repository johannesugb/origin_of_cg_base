...
namespace cgb {
	...
	vrs_eye_tracked_blit::vrs_eye_tracked_blit(std::shared_ptr<vulkan_command_buffer_manager> commandBufferManager, std::shared_ptr<vulkan_pipeline> pipeline,
	...
	void vrs_eye_tracked_blit::draw(std::vector<vulkan_render_object*> renderObjects, std::shared_ptr<cgb::vulkan_framebuffer> framebuffer)
	{
		...
		int32_t width = mVrsImages[vulkan_context::instance().currentFrame]->get_width();
		int32_t height = mVrsImages[vulkan_context::instance().currentFrame]->get_height();
		...
		commandBuffer.blitImage(
			...
			mVrsImages[vulkan_context::instance().currentFrame]->get_image(), vk::ImageLayout::eTransferDstOptimal,
			1, &blit,
			vk::Filter::eNearest);
	...
	void cgb::vrs_eye_tracked_blit::precompute()
	{

		auto width = mVrsImages[vulkan_context::instance().currentFrame]->get_width();
		auto height = mVrsImages[vulkan_context::instance().currentFrame]->get_height();
		auto mipLevels = mVrsImages[vulkan_context::instance().currentFrame]->get_mip_levels();
		auto numSamples = mVrsImages[vulkan_context::instance().currentFrame]->get_num_samples();
		auto format = mVrsImages[vulkan_context::instance().currentFrame]->get_format();
		auto tiling = mVrsImages[vulkan_context::instance().currentFrame]->get_tiling();
		mPrecomputedImage = std::make_unique<vulkan_image>(width * 2, height * 2, mipLevels, 1, numSamples, format, tiling,
