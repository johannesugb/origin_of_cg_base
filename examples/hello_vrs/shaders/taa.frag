...
void main() {
    ...
    vec3 c = texelFetchOffset(curFrame, ipos, 0, offset[0]).rgb;
    c = RGBToYCgCo(c);
    colorAvg += c;
    colorVar += c * c;
	c = texelFetchOffset(curFrame, ipos, 0, offset[1]).rgb;
    c = RGBToYCgCo(c);
    colorAvg += c;
    colorVar += c * c;
	c = texelFetchOffset(curFrame, ipos, 0, offset[2]).rgb;
    c = RGBToYCgCo(c);
    colorAvg += c;
    colorVar += c * c;
	c = texelFetchOffset(curFrame, ipos, 0, offset[3]).rgb;
    c = RGBToYCgCo(c);
    colorAvg += c;
    colorVar += c * c;
