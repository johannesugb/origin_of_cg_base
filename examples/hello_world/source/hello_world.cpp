...
class my_first_cgb_app : public cgb::cg_element
{
	...
	void createRenderPass() {
        ...
        colorAttachment.format = (VkFormat)cgb::context().main_window()->swap_chain_image_format().mFormat;
        ...
        if (vkCreateRenderPass(cgb::context().logical_device(), &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) {
 ...
 //   }
	...
	void initialize() override
	{
		...
		mPipeline = cgb::make_shared(cgb::graphics_pipeline_for(
			"shaders/a_triangle.vert",
			"shaders/a_triangle.frag",
			cgb::viewport_depth_scissors_config::from_window(cgb::context().main_window()),
			cgb::renderpass(cgb::renderpass_t::create(renderPass, (VkFormat)cgb::context().main_window()->swap_chain_image_format().mFormat))
		...
		for (auto i = 0; i < mCmdBfrs.size(); ++i) { // TODO: WTF, this must be abstracted somehow!
			...
			cmdbfr.begin_render_pass(renderPass, cgb::context().main_window()->backbuffer_at_index(i).handle(), { 0, 0 }, extent);
			//cmdbfr.handle().bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipe.layout_handle(), 0u, { }, {});
			cmdbfr.handle().bindPipeline(vk::PipelineBindPoint::eGraphics, cgb::get(mPipeline).handle());
